    ...
    <div className="space-y-4">
      ...
      <div className="card overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {clients.length === 0 && <tr><td colSpan={5} className="px-4 py-12 text-center text-zinc-400">{m.fabr.empty}</td></tr>}
            {clients.map((c) => (
              <tr key={c.id} className="border-b border-zinc-100 hover:bg-zinc-50">
                ...
                <td className="px-4 py-3 text-zinc-600">{c.address || "—"}</td>
        ...
        <Modal open={true} title={editClient ? m.fabr.edit : m.fabr.addClient} onClose={() => { setShowForm(false); setEditClient(null); resetForm(); }}>
          <form onSubmit={handleSave} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><label className="mb-1 block text-sm font-medium text-zinc-700">{m.fabr.address}</label><input className="input" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} /></div>
            </div>
